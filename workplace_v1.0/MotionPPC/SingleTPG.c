...
int TPGen_4Order_P2P(const s_TParam *TP, double *dd, double *Td, double *Tj, double *Ta, double *Tv, int cal_d_j)
{
	double d =TP->d;
	double t1, t2, t3, t4;
	double P,Q,D,R;
...
//		 TP.Ts = DEFAULT_TIMEINTERVAL;
   ...
   // TP.p = DEFAULT_DISPLACEMENT;*/
	...
	{
//		printf("hah\n");
		logMsg( "ERROR: Negative Input Found!\n",0,0,0,0,0,0);
		return e_ERRINPUT;
	}
...
//acceleration test   为5555555.
   ...
   // double x, ddq, pp, dif, cor1, cor2, tt;
    ...
    if(TP->Ts > 0)
    {
		...
		ti =      (tt / TP->Ts);				 //should be integer number of samples
...
/*	XExpoParam[0]= TP->Ts;
 ...
 /*           GetRunData(&Machine.coarseStage.arrAxis[0].sTP_Run,XExpoParam);
			...
			GetRunData(&Machine.coarseStage.arrAxis[2].sTP_Run,XExpoParam);
			GetRunData(&Machine.coarseStage.arrAxis[3].sTP_Run,XExpoParam);*/
