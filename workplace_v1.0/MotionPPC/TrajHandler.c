...
{
//Coarse_home
 // if(Machine.HomeFlagCorse ==1)

  if(pAxis->HomeFlagCorse ==1.0)
  {
//	  comData.iFineData[16] =10;
	  //上轴
      if( Machine.stepFlagup == 1)
      {
		 	EnableIntr3();
...
{
	double p_dd,Td, Tj, Ta, Tv;
	int CAL_MIN_J_AND_D =1;
	double posDistance;
	double ExpoParam[10];
//	printf("%lf",pTP->p);
	posDistance = pTP->p * 1000000000 - pAxis->dActPos;
//	printf("%lf\n",posDistance);
//	absolute(&posDistance);
	pTP->p = fabs(posDistance)/1000000000;
//	printf("%lf\n",pTP->p);
//	printf("%lf\n%lf\n%lf\n%lf\n",pTP->v,pTP->a,pTP->j,pTP->d);
	TPGen_4Order_P2P(pTP, &p_dd, &Td, &Tj,&Ta, &Tv, CAL_MIN_J_AND_D);
	pTP->p = (posDistance + pAxis->dActPos)/1000000000;
